...
releaseBtn.addEventListener("click", async () => {
    ...
    for (let utxo of scriptUtxos) {
        ...
        // Check if signer is owner and approvals are sufficient
        const approvalsCount = fdApprovals.length;
        if (approvalsCount >= fdRequiredApprovals && signerPkh === fdOwner) {
            goodUtxo = utxo;
            break;
        }
    }
    ...
    if (!goodUtxo) {
        alert("Cannot release: either not the owner or insufficient approvals");
        return;
    }
    ...
    const now = Date.now();
    if (now > Number(fdDeadline)) {
        alert("Cannot release: deadline has passed");
        return;
    }
    ...
    try {
        const tx = await lucid
            .newTx()
            .collectFrom([goodUtxo], Data.to(new Constr(2, []))) // Release redeemer
            .addSignerKey(signerPkh)
            .attachSpendingValidator(validator)
            .payToAddress(owner, { lovelace: goodUtxo.assets.lovelace })
            .validFrom(now) // current time
            .complete();
